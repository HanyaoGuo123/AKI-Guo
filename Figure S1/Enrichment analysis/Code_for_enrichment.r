...
setwd("D:\\肾损伤数据\\富集分析")
genes <- read.table('D:\\肾损伤数据\\富集分析\\GSE139061_DEGs.txt', header = TRUE )
...
write.table(enrich.go, 'GSE139061_GO.txt', sep = '\t', row.names = FALSE, quote = FALSE)  #output txt
...
#output pdf
pdf("GSE139061_GO.pdf",width=7,height=10)
cnetplot(enrich.go,circular=FALSE,colorEdge = TRUE,node_label = NULL,showCategory = 10)
dev.off()
...
write.table(kegg, 'GSE139061_KEGG.txt', sep = '\t', quote = FALSE, row.names = FALSE)
...
pdf("GSE139061_KEGG_result.pdf",width=7,height=8)
cnetplot(kegg,circular=FALSE,colorEdge = TRUE,node_label = NULL,showCategory = 10)
dev.off()
...
head(sortKEGG)
dim(sortKEGG)
write.csv(sortKEGG,'GSE139061_GSEA_KEGG.csv',row.names = FALSE) 

pdf("GSE139061_GSEA_KEGG.pdf",width=8,height=10)
...
write.csv(sortGO,'GSE319061_GSEA_GO.csv',row.names = FALSE)

pdf("GSE139061_GSEA_GO.pdf",width=8,height=10)
